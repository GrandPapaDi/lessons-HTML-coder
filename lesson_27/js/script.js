...
function documentActions(e) {
  ...
  if (targetElement.closest("[data-spoller]")) {
    ...
    if (!currentElement.nextElementSibling.classList.contains("--slidding")) {
      currentElement.classList.toggle("active");
    }
    //Регулюємо затримку
    slideToggle(currentElement.nextElementSibling, 800);
  }
}
...
let slideDown = (target, duration = 500) => {
  if (!target.classList.contains("--slidding")) {
    target.classList.add("--slidding");
    target.hidden = false;
    ...
    setTimeout(() => {
      ...
      target.classList.remove("--slidding");
    }, duration);
  }
};
